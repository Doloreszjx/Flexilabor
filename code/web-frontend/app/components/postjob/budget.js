    ...
    <div>
      ...
      <div className="mb-6">
        <label className="flex items-center text-gray-700">
          <input
            ...
            onChange={(e) => setBudget({ ...budget, negotiable: e.target.checked })}
            className="mr-2"
          />
          Negotiable Budget
        </label>
      ...
      <div className="mt-6 flex justify-end">
        <button
          ...
          disabled={!(budget.min && budget.max && !isNaN(budget.min) && !isNaN(budget.max) && parseFloat(budget.min) <= parseFloat(budget.max))}  >
          Submit
        </button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg">
            <h3 className='text-lg text-[#01ABF0] font-bold mb-4'>Posting a job costs $5.</h3>
