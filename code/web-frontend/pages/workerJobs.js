...
function WorkerJobs() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URI}/offlinejobs`);
        setJobs(response.data.data.filter(el => el.postedBy.role === 'Contractor'));
        setFilteredJobs(response.data.data.filter(el => el.postedBy.role === 'Contractor'));
        setLoading(false);
      } catch (err) {
