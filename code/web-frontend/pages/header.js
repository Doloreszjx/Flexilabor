...
function Header() {
	...
	// fixed bug: 初始化后得到的auth并不是用户的信息，而是一个用于管理用户身份验证的 Firebase 身份验证对象
			...
			<div className='mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8'>
				<div className='flex h-16 items-center justify-between'>
					<div className='md:flex md:items-center md:gap-12'>
						...
						>
							{/* Logo  */}
							<Image src='/logo.svg' alt='Logo' width={200} height={33} />
						</a>
					</div>
							...
							<ul className='flex gap-6 items-center text-sm'>
								{menuItems.map((el, index) => (
									<div key={index}>
										<li>
											<a
												className='text-black text-lg hover:text-[#1D90F1] transition'
												href={el.link}
											>
												{' '}
												{el.label}{' '}
